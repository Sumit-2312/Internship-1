    ...
    <div className="w-full h-fit bg-gray-500 grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-3 p-4 rounded-2xl">
      ...
      {Posts.map((Posts:any) => (
       <Link to={`/post/${Posts.id}`} >
          <div 
            key={Posts.id}
            className="relative group h-96 bg-gray-300 hover:cursor-pointer hover:scale-105 transition-transform duration-300 ease-in-out rounded-2xl shadow-lg shadow-gray-800 overflow-hidden"
          >
